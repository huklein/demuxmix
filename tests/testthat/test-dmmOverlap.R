test_that("dmmOverlap calculates shared probablity mass accurately", {
  ...
  mm <- demuxmix:::NaiveMixModel(mu1=100,
                                 mu2=150,
                                 theta1=20,
                                 theta2=5,
                                 pi=c(0.5, 0.5),
                                 hto=c(rnbinom(100, mu=100, size=20),
  ...
  dmm <- demuxmix:::Demuxmix(models=list(mm),
                  outliers=matrix(NA),
                  clusterInit=matrix(NA),
                  posteriorProb=matrix(c(rep(0, 100), rep(1, 100)), nrow=1,
  ...
  expect_equal(dmmOverlap(dmm, tol=0.8), 0.5277511, tolerance=0.8/0.5277511)
  expect_equal(dmmOverlap(dmm, tol=0.1), 0.5277511, tolerance=0.1/0.5277511)
  expect_equal(dmmOverlap(dmm, tol=0.001), 0.5277511, tolerance=0.001/0.5277511)
